import { Loader2, Pause, Square, GripHorizontal, Play } from 'lucide-react'
...
export function FloatingStatus({ isDownloading, title, progress, current, total, isDrawerOpen, onPause, onCancel, onResume }: FloatingStatusProps) {
    ...
    <div ref={dragRef} onMouseDown={handleMouseDown} className={className} style={style}>
      <div className={`rounded-xl bg-gray-800/90 p-4 shadow-2xl backdrop-blur-md ring-1 ${isDragging ? 'ring-blue-400' : 'ring-blue-500/30'} transition-shadow`}>
        ...
        <div className="flex items-center justify-between mb-2 select-none">
          <div className="flex items-center gap-3 overflow-hidden flex-1 min-w-0">
            {isDownloading ? (
               ...
               <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-yellow-500/20 text-yellow-400">
                 <Pause size={18} />
               </div>
            )}
            
            <div className="flex flex-col min-w-0">
              <span className={`text-xs font-bold ${isDownloading ? 'text-blue-400' : 'text-yellow-400'}`}>
                  {isDownloading ? `다운로드 중... (${current || 1}/${total})` : '일시정지됨 / 대기 중'}
              </span>
          ...
          <div className="flex items-center gap-3 shrink-0 ml-4">
             ...
             <div className="flex items-center gap-1">
                {isDownloading ? (
                    <>
                      {onPause && (
                          <button onClick={(e) => { e.stopPropagation(); onPause(); }} className="p-1.5 rounded-full hover:bg-white/10 text-yellow-400 transition" title="일시정지">
                            <Pause size={16} fill="currentColor" />
                          </button>
                      )}
                      {onCancel && (
                          <button onClick={(e) => { e.stopPropagation(); onCancel(); }} className="p-1.5 rounded-full hover:bg-white/10 text-red-400 transition" title="취소">
                    ...
                    // 일시정지 상태일 때 재개 버튼
                    onResume && (
                        <button onClick={(e) => { e.stopPropagation(); onResume(); }} className="p-1.5 rounded-full hover:bg-white/10 text-green-400 transition" title="다시 시작">
        ...
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-700">
          <div 
            className={`h-full transition-all duration-300 ease-out ${isDownloading ? 'bg-gradient-to-r from-blue-500 to-cyan-400' : 'bg-yellow-500'}`}
